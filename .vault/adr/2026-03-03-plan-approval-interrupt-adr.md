...
# `plan-approval-interrupt` adr: `adr-20` | (**status:** `accepted`)
...
- Original ADR number: `ADR-20`
- Original title: `Plan Approval Interrupt`
...
- LangGraph docs — Approval Node with Interrupt
  — dedicated node calling `interrupt()` unconditionally; `Command(goto=...)` routing
- legacy-audits/2026-03-03-vaultspec-rule-drift.md — D-02
...
- ADR-019 — TeamState field patterns; `NotRequired` precedent
- ADR-022 — `validation_errors` FINISH gate; gate ordering reference
- ADR-023 — phase prerequisite gate at plan→exec boundary (complementary)
- ADR-025 — review artifact FINISH gate; gate ordering reference
- legacy-research/2026-03-03-plan-approval-interrupt-research.md — interrupt mechanism analysis, option comparison, resume flow, autonomous interaction
