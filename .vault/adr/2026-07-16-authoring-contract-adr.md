...
A sibling near-duplication was triaged in the same review: the harness rules-presence signal exists as the bundled-aware `_rule_content_resolves` leg in `src/vaultspec_a2a/context/harness.py` and as the deliberately bundled-blind `RuleManager.has_workspace_rules` in `src/vaultspec_a2a/context/rules.py`. These are a semantic fork, not copies — one asks whether the run can resolve any rule content, the other whether the workspace corpus alone is present — and the latter's docstring still names itself the harness signal, a claim the bundled-aware rules-leg ruling superseded. This record decides which predicate survives.
...
- `RuleManager.has_workspace_rules` has zero consumers outside its own tests; the provision verb (`cli/provision.py`) uses `verify_harness` for its verify-only path and needs no workspace-only probe. P01.S02's intent — surfacing rules absence as an eligibility reason instead of a silent `compile()` no-op — is fully satisfied by the `verify_harness` rules leg.
...
- For the rules-presence fork, keeping `has_workspace_rules` as a provisioning-status probe — rejected: its only plausible consumer, the provision verb, already uses `verify_harness` and needs no workspace-only signal, so keeping it preserves a fork-prone parallel predicate with a stale self-description and no caller.
- Rewiring `verify_harness` onto `has_workspace_rules` — rejected: silently reverts the bundled-aware rules-leg ruling; bundled-only (Path B) workspaces would refuse again.
...
For the rules-presence fork: `verify_harness`'s bundled-aware `_rule_content_resolves` remains the sole rules-eligibility predicate, and `RuleManager.has_workspace_rules` is removed together with its tests as dead code — it has no consumer, its docstring's harness claim was superseded, and its rules-absence-surfacing intent (P01.S02) is already delivered by the verifier leg, so the deletion loses no capability.

Migration lands in three isolated commit groups, never combined: first the additive contract module with its unit test and the data-sync test (touches no contested file); then one consumer file per commit (`team_config`, `run_start_policy`, `compiler`, `worker`, plus the `has_workspace_rules` removal in `context/rules.py` — each a small mechanical diff); finally the test files holding hardcoded role lists.

## Rationale

The knockout criterion is cycle-proofness by construction: every candidate home except a zero-internal-import leaf either deepens the graph/team bidirectional coupling or inverts the control layering, and the codebase's existing lazy-import scars show how costly a new cycle is. Among leaf placements, `authoring/` wins on domain fit — the contract literally enumerates who authors through engine proposals, which is that package's subject. Code-truth with a sync test wins over data-derivation because the roles set gates run-start refusal and token coverage; policy must not be mutable by a workspace file that agents and operators can override. Deleting the private worker constant (rather than exporting it) is what permanently retires the fragile gateway-to-worker cross-import: any branch still carrying it fails loudly with an ImportError at rebase and migrates to the contract, instead of silently drifting. Deleting `has_workspace_rules` rather than keeping a second predicate follows from the same driver as the whole record: one fact, one definition — a consumerless parallel predicate with a stale self-description is exactly how the next divergence starts. Grounding: the review-fanout findings and the harness eligibility work recorded under the agent-harness-provisioning ADR and the graph-agent-framework-harness execution records.
...
- Binding decision (c): the harness rules-eligibility predicate is the bundled-aware `verify_harness` leg (`_rule_content_resolves`); workspace-only rules-presence is a provisioning-status question, never an eligibility input — and since it had no consumer, `RuleManager.has_workspace_rules` was removed as dead code rather than kept as a fork-prone parallel predicate. P01.S02's rules-absence-surfacing intent is satisfied by the verifier, so the deletion loses no capability.
- Gains: one edit point for the next authoring topology or role; the gateway/api layer gains a public, stable import target; four code copies and the test copies collapse; the rules question has exactly one answering predicate.
- Costs accepted: one more top-level contract module to know about; the `authoring` package now carries vocabulary alongside its HTTP client (a deliberate scope widening); campaign branches holding the old private constant must rebase through an intentional ImportError; a future genuine need for a workspace-only provisioning probe re-adds it fresh under this record's terms (status signal, never eligibility).
