...
## Amendment - grounding gains a fourth, outbound leg (2026-08-01)

The three legs above deliver internal grounding: the vault, the codebase, and
the workspace filesystem. That scope no longer covers the need - document agents
authoring in 2026 must reach material that postdates any model's training and
lives on no local disk - so grounding gains a fourth, outbound leg.

The leg is delivered by provider-native web tools over the seams this record
already established: the exact-name autonomous allowlist that admits the native
read built-ins, and the per-run Codex config home that already carries that
lane's declared servers. No registry entry, no vendor, and no credential is
added; the closed registry stays closed. The registry's trust-root marker does
change: local write and network reach are independent properties, and only the
first was expressed, so egress becomes its own declared axis on the same
fail-loud, unsafe-by-omission terms.

The decision itself - tool admission, role scope, per-lane posture, evidence
