...
**A THIRD, from F50: test data that shares the code's assumptions cannot
falsify them.** Narrowing a provider identifier to its enumeration would have
passed every test written against fresh data and failed only when reading an OLD
run - one frozen under a provider this build no longer knows. The fixtures are
generated by the build that holds the assumption, so they can never contain the
counter-example. Where a field carries values RECORDED IN THE PAST rather than
produced now, its value set must be sampled from real historical data, never
from anything the current build generates.
...
FOURTH CORRECTION - THE SHAPE-ONE LIST IS INCOMPLETE, not merely partly misfiled.
F53 and F54 record two further vocabularies that discard an existing owning
enumeration and appear nowhere above. Treat the lists in this entry as a
STARTING POINT, never as a closed set: a remediation driven from them alone will
leave discarded enumerations in place. The measurement that produced this entry
counted what it looked at, and the sweep that corrected it looked wider.
...
### F48-shape-two-misclassified-four | medium | four vocabularies filed as undeclared already had owning types - CORRECTION APPLIED IN F23

Not a new defect. Applied in place in F23 rather than held, because leaving a
known-wrong classification standing in a document people execute against is
worse than an untidy trail.

`RepairStatus` (seven members, its docstring stating it types BOTH the repair
status and execution readiness columns), `ProviderCondition` (nine members, each
documenting its remedy) and `ApprovalStatus` all exist. The four were SHAPE ONE
- declared and discarded - not shape two. CONSEQUENCE, and the part worth
keeping: the record overstated how much vocabulary had to be INVENTED and
understated how much was already owned and merely not served - the difference
between a design task and a one-line annotation. It also hid that these four
were the CHEAPEST AND SAFEST of the set, because a vocabulary with an existing
owner has an existing write path to prove containment against. All four are now
served as their owning enumerations.

### F49-degraded-reasons-is-two-fields | medium | one name, two vocabularies, one of them not finite - CORRECTION APPLIED IN F23

Not a new defect; applied in place in F23. The run snapshot's field carries
machine tokens appended at SEVENTEEN sites across three modules. Service state's
carries operator sentences, and one member is built by interpolation from
another field - so THAT FIELD'S VALUE SET IS NOT EVEN FINITE. CONSEQUENCE: a
remedy phrased against the field NAME would either try to type an unbounded
prose channel, which cannot be done, or merge two vocabularies sharing nothing.
The run half now has a seventeen-member declaration; the service half is
free-form by design under the vocabulary record's enum-worthiness clause and is
documented as such at the field.

### F50-provider-id-is-not-the-provider-concept | medium | narrowing would break a read path, and only when reading an OLD run - CORRECTION APPLIED IN F23

Not a new defect; applied in place in F23. Two values are provably outside the
provider enumeration: the empty string, emitted and documented at the role
summary, and a provider this build no longer knows, whose lookup error is caught
with the comment that the truthful verdict is "not ready", not a crash on a read
path.

THE METHOD LESSON, which is why this entry is worth more than its severity:
narrowing here would have failed ONLY when reading an old run, so it would have
PASSED EVERY TEST WRITTEN AGAINST FRESH DATA. Test data generated by the same
build that holds an assumption cannot falsify that assumption. REMEDY: none to
the code - the typing is correct - and the reconciliation is documented at the
model so the next reader does not re-attempt it.

### F51-execution-readiness-has-no-write-gate | medium | one of two columns sharing a type is coerced on write and the other is not, and the failure now surfaces at the wire

LATENT, not a live breach: every current caller passes a valid member. Both
columns are typed by the same enumeration. Writes to the repair-status column
pass a coercion helper that raises with the valid set named; writes to the
execution-readiness column do not - two assignment sites take a raw value under
a signature admitting either the enumeration or a bare string. CONSEQUENCE, made
SHARPER by the remediation rather than milder: an out-of-vocabulary string
reaches the column silently, and now that the field is SERVED as an enumeration
the failure surfaces at the WIRE - a 500 on run-status for a value written
without complaint days earlier. CLASSIFICATION: genuine defect, latent. REMEDY:
route those assignments through the same coercion. NOT DONE - the owning module
is dirty under a concurrent session, and the agent reported rather than reaching
into it.

### F52-semantic-status-has-no-producer | medium | the field is permanently its default because nothing ever writes it

Two start-surface fields default to a starting value, and a sweep of every write
site finds NONE. Both are permanently that value on every run, including one
that fails during dispatch. CONSEQUENCE: a client reading it learns only that
the field exists - and because the default is a plausible value, that is
indistinguishable from a real one, which makes it WORSE than an absent field.

CLASSIFICATION: NOT a typing defect and NOT fixed by the vocabulary work. It is
now typed by the same vocabulary its sibling phase field answers from, since it
is that question asked one moment earlier - which constrains its domain and
changes nothing about its truth. This is the OBLIGED-WRITER problem, the same
family as the tool-call status that never advances, and it is ROUTED TO THE
STATE-TRUTHFULNESS RECORD's second clause rather than to the vocabulary record.
It is the second finding to land on that record after it was written, the first
being the terminal-partition question in F45.

### F53-role-capability-discards-the-model-enum | low | a served field drops an owning enumeration that F23 did not list

The role summary's capability field is served as a nullable string while its
emit site reads the value off an owning enumeration. Live serves a real member.
Shape one - and NOT IN F23's LIST, which is this finding's point: a remediation
working from that list alone would leave it. CONSEQUENCE: F23's shape-one
inventory is INCOMPLETE as well as partly misfiled, so the list is a starting
point rather than a closed set. CLASSIFICATION: genuine defect. Deliberately not
fixed - outside the executing Step's scope, and reported rather than quietly
widening a Step.

### F54-assignment-source-discards-its-enum-and-restates-its-default | low | an owning enumeration exists and the field is a bare string whose default is restated at two sites

A five-member enumeration exists, with a docstring explaining why its undeclared
member is deliberately not folded into the team-default member. The served field
is a bare string whose default is a literal, RESTATED again as a fallback at the
emit site. Live serves three distinct members. Shape one PLUS a direct violation
of the vocabulary record's derive-do-not-restate clause, and the two
restatements can drift independently of each other and of the enumeration.
CLASSIFICATION: genuine defect. Not fixed, same scope reasoning as F53.

### F55-supported-capabilities-undeclared | low | an unconstrained string array with a single producer

Shape two, CONFIRMED against the code. A string list with no owning type, one
producer keyed on topology, serving three members for one topology and empty for
the other three. CLASSIFICATION: genuine defect. Left undeclared because its
producer sat in a gated file; closes with the Step that owns it. This is the
same producer F7 and F37 record from the served side.

### F56-two-modules-exceed-the-size-mandate | low | pre-existing, and enforced by no gate

Two modules exceed the repository's module-size mandate; both predate this
campaign. CONSEQUENCE: the mandate is enforced by NO GATE, so a module crosses
it silently and only a reader notices. REMEDY: the vocabulary work routed its
new declarations into small owning modules following an existing precedent, so
its own growth is annotations and documentation rather than new types - but
