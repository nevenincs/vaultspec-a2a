...
### ER15-windows-shutdown | high | CLOSED

**CLOSED by W04.P10.S47 and W04.P10.S49; correctness/portability; A26; measured M09.** S47 replaced process-directed Windows termination with the owned cooperative Uvicorn transition. S49 closes admission first and carries one monotonic deadline through real-socket connection and stream drain, active work, bridge and worker cleanup, exact descendant reaping and bounded escalation. Formal PASS `4b2e0a61` accepts the final assignment-before-authority correction after FAIL reviews `dcac3b27` and `91c882fe`. Real late-child, failed-assignment and exact-handle/PID-reuse discriminators end with zero survivors. S48 discovery and S50 frozen-worker proof remain separate; provider containment W04.P11.S60 remains reopened.

### ER16-unbounded-graceful-stream-wait | high | CLOSED

**CLOSED by W04.P10.S49; lifecycle/boundedness; A26.** Uvicorn connection drain and application lifespan share the same absolute deadline. A parked real-socket SSE reached lifespan and the server exited in 1.25-1.32 seconds against a 3.0-second total with a 1.0-second stream grace. Active background owners and forced cleanup consume only remaining time; no stream wait resets or escapes the clock. Formal PASS `4b2e0a61` accepts the complete correction.
...
### ER22-upstream-testclient-deprecation | low | CLOSED

**CLOSED by W01.P02.S08; upstream maintenance; M02 warning.** The locked baseline Starlette 1.6.0 emits DeprecationWarning at `testclient.py:53` for `anyio.abc.BlockingPortal`; the exact warning-as-error import fails. PyPI has no later release containing the fix. S08 locks official merged Starlette commit `bbee894422c6cc1306327335ae385b901ccfec13`, whose TestClient uses `anyio.from_thread.BlockingPortal` at all three annotation sites. After sync, the warning-as-error import and real GET pass, the deprecated access scan is empty, and the canonical scan finds exactly three accesses. No filter, downgrade, shim or old API remains in the supported locked binary lane. Formal PASS `333080e4a351b9450158c8b8375d9beb301a98a1` accepts the correction for the uv-locked Dashboard-embedded binary. The expanded unreleased-tree risk remains MEDIUM/open pending an official release, and independently pip-resolved wheels remain outside the supported product contract; both boundaries stay explicit in the remediation audit, reference and S08 Step Record.
...
### ER21 corrected phase-boundary evidence
...
The corrected probe freezes duration and gap at one exact timestamp for each independent phase. Its first five-trial `C=5` rerun found compile gaps 0.0546-0.1796 seconds, all below the unchanged 0.5-second ER21 ceiling. The same run separately found a 3.6920-second loop gap during a 7.1849-second `WorkerBridge.close()` in trial four; checkpointer and ambient scheduler windows remained below 0.018 seconds. This is a confirmed production lifecycle observation assigned to `W04.P10.S49`, not compile evidence. The corrected S07 module passed 5 tests in 139.74 seconds. A separate current-schema restart/catalog run passed in 51.63 seconds without shutdown failure. Formal PASS `6c742790f42b21d433c371ed6db738c866d0fcd7` accepted the compile-window contract. S49 later corrects the teardown stall and closes it through formal PASS `4b2e0a61`.

## S49 lifecycle closure
...
The bridge retains an undelivered terminal event only in memory and returns false. This is not durable delivery or settlement; HIGH correction remains open at `W02.P03.S14`. Real process tests cover Job Object containment and the narrow exact-identity fallback before assignment authority exists. The first uncontained run exposed a surviving child after cooperative root exit; subsequent mandatory containment and exact retained-handle corrections cover late descendants and PID reuse. Corrected variants finish inside one four-second deadline with no survivor. S50 still owns full frozen-worker proof. S47 supplies the separately closed cooperative trigger; S48 discovery is unchanged.
...
S49's final implementation review additionally bounds cancellation/join of an already-running bridge flush and restores its extracted batch on cancellation during HTTP or retry backoff. A malformed worker lifecycle owner now fails 503. The final shared gate passes 48 tests in 41.39 seconds; worker app/IPC coverage passes 35 tests in 12.76 seconds. Formal PASS `4b2e0a61` accepts these corrections; they do not alter the open S14 durability boundary.
Formal S49 FAIL `dcac3b27` proved an initially uncontained root could create a child during its shutdown request and exit before the pre-request identity snapshot covered it. The correction makes containment mandatory for every gateway spawn and seats any restored live uncontained handle before offering cooperative shutdown; failed seating skips cooperation and escalates while the root remains live. Real initially uncontained/then-seated and precontained variants create their only child inside `/admin/shutdown`, return 202, exit the root and finish with zero survivors in 1.65 and 1.60 seconds inside one four-second deadline. The 16-test ownership gate passes in 18.75 seconds. Formal PASS `4b2e0a61` accepts the correction after the later assignment-before-authority fix.
...
### S49 assignment-before-authority correction

Formal rereview `91c882fe` exposed a HIGH pre-admission leak when Windows Job assignment failed before containment recorded a pid: empty-containment termination returned success, the retained root wait expired after 5.0086 seconds and the root survived. S49 now falls back to the exact live `Popen` identity only when containment was never assigned. It caches psutil creation identity, suspends the retained root while collecting its descendants, terminates only those scoped identities, waits the original handle and propagates the assignment failure. No host-wide scan or post-exit bare-pid action is used.

A real closed-Job discriminator starts one root and two 300-second descendants, drives production readiness before admission and completes in 0.59 seconds with the original `ProcessContainmentError` and zero survivors. Its first invalid attempt retained the uv venv redirector and exceeded 60 seconds; that owned test tree was stopped with zero residue, and the fixture was corrected to use the base interpreter. The focused containment result is 24 passed in 12.09 seconds; the full S49 result is 68 passed in 50.17 seconds with Ruff and Ty clean.

Formal PASS `4b2e0a61` accepts correction `d8453cf0` and the full implementation chain `8f79c919` -> FAIL `dcac3b27` -> `84ba2a2b` -> FAIL `91c882fe` -> `d8453cf0`. Core closes only S49 and ER15/ER16. The 7.1849-second bridge-close / 3.6920-second loop stall, parked SSE, late child, assignment failure, exact-handle/PID-reuse guard, harness hangs and final 68-pass evidence remain preserved above. Volatile terminal delivery remains HIGH/open under W02.P03.S14. The 90-second recovery polling hang remains HIGH/open under served-capability W04.P08.S56 and remediation W02.P03.S11. Trace-only `141147db` leaves provider containment W04.P11.S60 reopened. S48 and S50 remain separate.
