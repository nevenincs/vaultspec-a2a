...
date: '2026-07-14'
modified: '2026-07-15'
body_hash: 'sha256:d49e6a47cc2fd95ee7fb401dc0197243506d111db930fe9edd9915ae02d521dc'
step_id: 'S08'
related:
...
The engine now publishes the full review lifecycle to the durable outbox (dashboard commits `5173858` then `a7ad6f3`, "publish/extend review lifecycle events to the durable outbox"). Re-verified against a freshly built binary at `a7ad6f3` served as a second workspace-local instance, attach-never-own, on an ephemeral port distinct from the owner's running engine.
...
- `approval.resolved` `decision=request_changes` `comment=tighten it` -> verdict `(request_changes, "tighten it")`, correlated to `thread-edit`.

Wire contract pinned at `a7ad6f3`: approve AND request-changes both ride `approval.resolved`, disambiguated only by the authoritative `data.decision` field (`approve` vs `request_changes`); reject rides `proposal.rejected`. Each decision payload carries `{decision, comment, proposal_id, changeset_id, approval_id, resulting_status, resulting_revision}`. The decoder keys the decision field first, so a non-verdict transition such as supersede (`proposal.updated` with no `decision`) stays correctly non-resolving. The decision endpoint envelope command is the `CommandKind` (`approve`/`reject`/`edit_proposal`) while the payload decision is the `ReviewDecisionKind` (`approve`/`reject`/`edit`); `reviewed_revision` is the submit receipt's `approval.reviewed.proposal_revision`.
