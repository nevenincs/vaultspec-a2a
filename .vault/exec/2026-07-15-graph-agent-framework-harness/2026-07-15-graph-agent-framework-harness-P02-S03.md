...
date: '2026-07-16'
modified: '2026-07-16'
body_hash: 'sha256:b34b2f8d7f348507f03aa2477502f78a3c611fa2fee8ad67814776e690d7b06f'
step_id: 'S03'
related:
...
Review closure (LOW-4): the two-source cache-invalidation edges - a bundled file disappearing while the bundled source is set, and a workspace change while bundled is also set - are pinned by `TestCompileOrderAndCacheEdges` (landed with `8ebbe26`) plus `TestBundledDefaults::test_bundled_dir_change_invalidates_cache`, covering both the `_has_changes` candidate-dirs union (tier 1) and the per-file mtime tier (tier 2) across both sources. No further tests were owed; a duplicate draft was discarded on discovery.
...
**Arbitration addendum (same day):** team-lead posed the alternative - now that a one-command `workspace provision` verb exists, require provisioned workspaces for document runs and demote Path B to freshness/shadow semantics (an owner-sign-off reversal). REJECTED on a decisive verified fact: the provisioned `.vaultspec/rules` corpus contains ZERO `roles:`-tagged files (checked against this repo's own fully-provisioned corpus), and the provision verb installs the vaultspec-core corpus, never the a2a package-data conventions file - so under role-scoped compile, document turns receive their rules from the BUNDLED dir in EVERY workspace, provisioned or not. The workspace-rules probe therefore verifies a corpus document-role turns do not consume; mandating provisioning would enshrine that mismatch, not fix it. The delegate-to-RuleManager fix is the only shape consistent with the provisioning ADR's own Verification-not-hope clause (verify the actual delivery path). Note the practical eligibility outcome for bare workspaces is unchanged either way - the templates/skills/CLI legs still refuse them - so no Path B reversal arises and no owner sign-off is needed; the fix changes what is verified and what reason is served, not who gets refused.
