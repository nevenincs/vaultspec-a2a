...
date: '2026-07-16'
modified: '2026-07-16'
body_hash: 'sha256:f2d25b11d6453e2c3cc82d619a191350ea2d9c85a818c793e99c85d4445d4d5f'
step_id: 'S04'
related:
...
Circular-import attribution (team-lead's note): the `context/__init__` -> `token_budget` -> `thread` -> `graph` -> `supervisor` -> `context.token_budget` cycle that makes `test_rules.py` uncollectable in isolation is the SAME `context/token_budget` cycle a prior session (opus-8) flagged - two independent hits now, worth the owner's attention. Still not fixed here (out of P02 scope); the S03 bundled-read (e975850) composes on top of this S04 filter (bundled files carry the same `roles:` frontmatter through the identical path).
