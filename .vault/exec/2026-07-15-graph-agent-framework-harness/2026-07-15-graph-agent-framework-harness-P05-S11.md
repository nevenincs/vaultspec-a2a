...
The ADR's tripwire wording referenced a `has_workspace_rules()` gate; no such
function exists in the code today, so the tripwire is asserted behaviourally (a
bare bundled-only workspace is not refused and still yields conventions), which
is the guarantee the tripwire intends. The workspace is a bare on-disk tmp dir
rather than the engine-`provisioned_workspace` fixture, because Path B requires
the absence of `.vaultspec/rules` that the provision verb would scaffold.
