...
date: '2026-07-17'
modified: '2026-07-17'
body_hash: 'sha256:411f2243d1287bd20aab4d695d5a1847bcb889a9b49950049a94dda40ff13edd'
step_id: 'S10'
related:
...
- ACCEPTED RESIDUAL - raw-name-match coupling: the exact-name auto-approve is only as precise as the name it matches on, and Kimi's permission title (`get_title()` = `f"{tool_name}: {subtitle}"`) carries NO server scope - just the bare tool name. Raw-name match is therefore the MAXIMUM achievable precision at this RPC layer; there is no server-qualified identifier to match against. This precision is load-bearing on two invariants held elsewhere: (a) P03.S12's ambient-MCP isolation (the `--config '{"mcpServers": {}}'` launch flag) ensuring no non-composed server's tools ever reach the session, and (b) the read-only registry trust-root (`_KNOWN_MCP_SERVERS` `read_only` guard) ensuring only read-only servers are ever composed in the first place. The residual: a hypothetical tool on a NON-composed server whose raw name collided with an approved read name (e.g. a rogue `Grep`) would be auto-approved by name alone - but only if isolation (a) were bypassed, since an uncomposed server contributes no tools to the session under (a). The coupling is accepted, not mitigated, because raw-name is the ceiling Kimi's title format imposes; the defense is the isolation + trust-root invariants, not tighter matching.
