...
- [ ] `P01.S01` - Read the actual `DocumentProposalSubmitter` and phase-gate call sequence end to end and compare it against each of the four personas' instructed CLI/rag actions; `src/vaultspec_a2a/authoring/submitter.py`, `src/vaultspec_a2a/graph/nodes/phase_gate.py`, `src/vaultspec_a2a/team/presets/agents/vaultspec-researcher.toml`, `vaultspec-synthesist.toml`, `vaultspec-adr-author.toml`, `vaultspec-doc-reviewer.toml`.
- [ ] `P01.S02` - Record the resolved vestigial-vs-aspirational finding as a probe note fed into P02 and P03, naming exactly which persona instructions are dead prompt text and which need a built invocation path; `.vault/exec/2026-07-15-graph-agent-framework-harness/2026-07-15-graph-agent-framework-harness-P01-summary.md`.
...
- [ ] `P02.S03` - Extract the taxonomy/frontmatter/wiki-link/template conventions a document-authoring agent needs into a role-scoped rule source, separate from the full builtin corpus; `.vaultspec/rules/rules/` (new non-builtin rule file(s)), `.vaultspec/templates/adr.md`, `research.md`, `plan.md`, `audit.md`, `ref-audit.md`.
...
Executes the accepted `graph-agent-framework-harness-adr` (closing the framework-harness gap for graph-executed document-authoring agents), grounded in `graph-agent-framework-harness-research`. This plan is the live continuation of `universal-rule-propagation-adr` (ADR-028, now reconciled to accepted): ADR-028's `RuleManager` mechanism was built and wired, but two concrete gaps remain open - `include_builtin=False` excluding the mechanical builtin rule files at both call sites, and the four research_adr document personas' TOML prompts instructing CLI/rag invocations their `terminal=false` capability cannot execute.

Phase `P01` resolves the vestigial-vs-aspirational persona-prompt question with evidence before any other work proceeds, since the correct fix in `P03` (strip the instructions vs. build a missing invocation path) depends on the answer. `P02` designs a role-scoped rule-propagation shape that avoids ADR-028's own flagged token-inflation and cross-role-noise risk rather than reaching for a blanket `include_builtin=True`. `P03` reconciles each of the four personas' prompts against the `P01` finding. `P04` wires the `P02` shape into the two `RuleManager` call sites. `P05` proves the fix live against a real provisioned run workspace, per the ADR's explicit constraint that a static-repo check is insufficient.

Related to `adr-authoring-orchestration-adr`/`-plan` (the research_adr topology this harness gap lives inside) and `multi-provider-execution-adr`/`-plan` (the broader Program 3 mission this feature belongs to). Cross-referenced, not folded into, the parallel session's `agent-harness-provisioning-adr`/`-plan`: that plan covers the system-wide harness contract across all agent types; this plan is the narrower, code-verified fix for the research_adr topology's specific gaps.
...
- The `P01` probe note names a definitive vestigial-vs-aspirational finding backed by the actual `DocumentProposalSubmitter`/phase-gate call sequence, not assumed from prompt text alone.
- The role-scoped rule source lands as real files under `.vaultspec/rules/rules/`, readable and distinct from the full builtin corpus, with `RuleManager` discovery able to select a persona-scoped subset (not all-or-nothing).
- All four persona TOML prompts contain no instruction the runtime cannot execute; each prompt accurately reflects either a newly built invocation path or the real graph-driven propose/submit flow.
