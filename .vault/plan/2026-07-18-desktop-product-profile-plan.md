...
- [ ] `W01.P03.S110` - Resolve the target-selective Python closure from the committed uv lock and the ACP closure from the committed package lock acquire every artifact into the sha256-keyed content-addressed cache derive per-package license identity from metadata and tarball contents with committed curated overrides and emit the canonical closure inventories; `src/vaultspec_a2a/desktop/capsule_input_authoring.py, scripts/desktop_capsule_inputs.toml, src/vaultspec_a2a/desktop/tests/test_capsule_input_authoring.py`.
