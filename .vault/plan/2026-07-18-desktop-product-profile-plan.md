...
- [x] `W01.P03.S107` - Add the production installed-inventory builder that consumes verified archive sessions applies the layout authority and emits canonical v2 inventory bytes into the content-addressed input cache invoked by the capsule build script; `src/vaultspec_a2a/desktop/installed_inventory.py, src/vaultspec_a2a/desktop/tests/test_installed_inventory_builder.py`.
