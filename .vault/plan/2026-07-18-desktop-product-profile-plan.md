...
- [x] `W01.P03.S114` - Drop the Intel macOS target from the shipped capsule matrix across the contract enum published manifest schema declared inputs and certification workflow and regenerate the schema snapshot and golden vector; `src/vaultspec_a2a/desktop/contract.py, schemas/desktop-capsule-manifest.json, scripts/desktop_capsule_inputs.toml, .github/workflows/desktop-capsule.yml`.
- [x] `W01.P03.S113` - Extract the pure closure selection core from the lock reconciliation authority so marker evaluation extras propagation graph resolution and platform filtering have one definition and re-express both reconciliation entry points as comparisons against it; `src/vaultspec_a2a/desktop/lock_reconciliation.py, src/vaultspec_a2a/desktop/tests/test_lock_reconciliation.py`.
- [x] `W01.P03.S110` - Add the wheel selection authority deriving a deterministic per-target ordered supported-tag list from the pinned packaging dependency and ranking every admitted lock wheel by best tag index with a fixed vector guarding the derivation; `src/vaultspec_a2a/desktop/capsule_input_authoring.py, src/vaultspec_a2a/desktop/tests/test_capsule_input_authoring.py`.
- [x] `W01.P03.S115` - Acquire every wheel tarball license blob and pinned source into the sha256-keyed content-addressed cache verifying each byte against its committed pin and failing closed on any digest mismatch; `src/vaultspec_a2a/desktop/capsule_input_authoring.py, src/vaultspec_a2a/desktop/tests/test_capsule_acquisition.py`.
- [x] `W01.P03.S116` - Derive per-package Python license identity from wheel metadata with the committed curated overrides round-tripped through the real wheel verifier and emit the canonical Python closure inventory; `src/vaultspec_a2a/desktop/capsule_input_authoring.py, scripts/desktop_capsule_inputs.toml, src/vaultspec_a2a/desktop/tests/test_capsule_license_derivation.py`.
- [x] `W01.P03.S118` - Resolve the ACP license contract gap with a curated override path for non-SPDX SEE-LICENSE-IN references binding the referenced license file then derive ACP package license identity and emit the canonical ACP closure inventory; `src/vaultspec_a2a/desktop/closure_inventory.py, src/vaultspec_a2a/desktop/package_archives.py, src/vaultspec_a2a/desktop/capsule_input_authoring.py, scripts/desktop_capsule_inputs.toml`.
- [x] `W01.P03.S117` - Change the layout authority to drop wheel data headers and data scripts members deterministically with per-member evidence instead of rejecting them keeping data data platinclude and unknown keys fail-closed so the real closure materializes; `src/vaultspec_a2a/desktop/install_layout.py, src/vaultspec_a2a/desktop/tests/test_install_layout.py`.
- [x] `W01.P03.S111` - Open verified archive sessions build the installed inventories through the production builder author and digest the pinned capsule input descriptor naming every artifact and prove the retained input session envelope against a real per-target closure; `src/vaultspec_a2a/desktop/capsule_input_authoring.py, src/vaultspec_a2a/desktop/tests/test_capsule_input_authoring.py`.
- [x] `W01.P03.S112` - Reserve and place the donated launcher stub license notice inside the capsule so the redistribution terms ship with the product launchers; `src/vaultspec_a2a/desktop/capsule_assembly.py, src/vaultspec_a2a/desktop/capsule_materializer.py, src/vaultspec_a2a/desktop/tests/test_capsule_materializer.py`.
- [x] `W01.P03.S119` - Wire the production preparation orchestration entrypoint running the full per-target flow from acquisition through installed-inventory build to descriptor authoring and digest emitting the real shippable capsule input descriptor and confirming the retained session byte envelope against a real per-target closure; `src/vaultspec_a2a/desktop/capsule_input_authoring.py, scripts/prepare_desktop_capsule.py, src/vaultspec_a2a/desktop/tests/test_prepare_capsule.py`.
- [x] `W01.P03.S120` - Carry the dropped-member audit trail from the closure layout onto the installed inventory and the pinned descriptor so the build stage surfaces the data-headers and data-scripts omissions into published evidence as a read-only consumer without re-deriving the layout; `src/vaultspec_a2a/desktop/installed_inventory.py, src/vaultspec_a2a/desktop/artifacts.py, src/vaultspec_a2a/desktop/capsule_descriptor.py, src/vaultspec_a2a/desktop/capsule_preparation.py`.
- [x] `W01.P03.S13` - Assemble each deterministic target capsule directly inside one caller-owned final-name unpublished generation from verified retained Python Node ACP and package-owned inputs enforce aggregate dashboard path file license and cross-closure collision bounds and emit complete machine-readable installed-tree evidence without inner publication; `scripts/build_desktop_capsule.py`.
- [x] `W01.P03.S14` - Verify the complete caller-owned unpublished generation identity target closure entrypoints installed bytes digests licenses software bill of materials and evidence without a source checkout; `scripts/verify_desktop_capsule.py`.
- [x] `W01.P03.S15` - Publish only target-native unpublished generations and component evidence that passed producer verification for dashboard consumption; `.github/workflows/desktop-capsule.yml`.
...
- [x] `W01.P03.S100` - Consolidate package capsule and manifest archive scanning behind shared bounded member path ancestor collision expansion-ratio central-directory raw-tar and retained-session authorities before extraction; `src/vaultspec_a2a/desktop/_archive_authority.py, src/vaultspec_a2a/desktop/_capsule_archive_io.py, src/vaultspec_a2a/desktop/package_archives.py, src/vaultspec_a2a/desktop/capsule.py, src/vaultspec_a2a/desktop/manifest.py, src/vaultspec_a2a/desktop/artifacts.py, src/vaultspec_a2a/desktop/tests/test_artifacts.py, src/vaultspec_a2a/desktop/tests/test_package_archives.py, src/vaultspec_a2a/desktop/tests/test_capsule_archives.py, src/vaultspec_a2a/desktop/tests/test_manifest.py`.
- [x] `W01.P03.S101` - Retain one exact capsule descriptor both locks all four sources closure inventories package archives and external licenses through scope-bound input sessions and emit the component manifest from bound evidence without reopening mutable paths; `supported-target proof or a revised bound is still required for the 512-descriptor and 8 GiB retention envelope; `src/vaultspec_a2a/desktop/artifacts.py, src/vaultspec_a2a/desktop/manifest.py, src/vaultspec_a2a/desktop/tests/test_artifacts.py, src/vaultspec_a2a/desktop/tests/test_manifest.py, pyproject.toml, uv.lock`.
- [x] `W01.P03.S102` - Derive one immutable pre-mutation whole-capsule assembly plan that reserves every runtime package launcher lock license manifest evidence and archive path under dashboard ASCII ancestor collision file size and license bounds; `src/vaultspec_a2a/desktop/capsule_assembly.py, src/vaultspec_a2a/desktop/tests/test_capsule_assembly.py`.
- [x] `W01.P03.S105` - Add the pure wheel and npm install-layout authority mapping RECORD-verified archive members to installed destinations under the fixed closure roots with fail-closed unsupported-feature handling; `src/vaultspec_a2a/desktop/install_layout.py, src/vaultspec_a2a/desktop/tests/test_install_layout.py`.
- [x] `W01.P03.S106` - Extend the installed inventory to version two with per-file source_sha256 and source_member provenance proving every pair names a verified closure member and migrate all fixtures to real provenance; `src/vaultspec_a2a/desktop/installed_inventory.py, src/vaultspec_a2a/desktop/tests/test_installed_inventory.py`.
- [x] `W01.P03.S107` - Add the production installed-inventory builder that consumes verified archive sessions applies the layout authority and emits canonical v2 inventory bytes into the content-addressed input cache invoked by the input preparation authority; `src/vaultspec_a2a/desktop/installed_inventory.py, src/vaultspec_a2a/desktop/tests/test_installed_inventory_builder.py`.
- [x] `W01.P03.S103` - Materialize retained Python wheels the root A2A wheel ACP packages external licenses and relocatable target launchers through leased nested-directory authority and reconcile every written byte against installed closure evidence; `src/vaultspec_a2a/desktop/capsule_assembly.py, src/vaultspec_a2a/desktop/package_archives.py, src/vaultspec_a2a/desktop/tests/test_capsule_assembly.py, src/vaultspec_a2a/desktop/tests/test_package_archives.py`.
- [x] `W01.P03.S108` - Compose the Windows product launchers from the content-addressed console stub a relocatable launcher-dir shebang pinning the bundled interpreter and a deterministic epoch-stamped zip that pins the library root replacing the fail-loud Windows refusal and proving one composed launcher by live execution; `src/vaultspec_a2a/desktop/capsule_materializer.py, scripts/desktop_capsule_inputs.toml, src/vaultspec_a2a/desktop/tests/test_capsule_materializer.py`.
- [x] `W01.P03.S109` - Emit external-license provenance evidence through the production inventory builders so standalone license artifacts reconcile end-to-end without hand-extended evidence closing the capsule license compliance gap; `src/vaultspec_a2a/desktop/artifacts.py, src/vaultspec_a2a/desktop/installed_inventory.py, src/vaultspec_a2a/desktop/tests/test_installed_inventory_builder.py`.
...
- [x] `W03.P08.S98` - Replace platform-skipped credential permission and link tests with real host-native assertions that never skip or xfail; `src/vaultspec_a2a/desktop/tests/test_credentials.py`.
...
- [x] `W05.P13.S76` - Certify Apple Silicon macOS capsule closure and upload its pinned component contract; `.github/workflows/desktop-capsule.yml`.
- [x] `W05.P13.S77` - Certify Intel macOS capsule closure and upload its pinned component contract; `.github/workflows/desktop-capsule.yml`.
- [x] `W05.P13.S78` - Certify Arm64 Linux capsule closure and upload its pinned component contract; `.github/workflows/desktop-capsule.yml`.
- [x] `W05.P13.S79` - Certify x86-64 Linux capsule closure and upload its pinned component contract; `.github/workflows/desktop-capsule.yml`.
- [x] `W05.P13.S80` - Certify x86-64 Windows capsule closure and upload its pinned component contract; `.github/workflows/desktop-capsule.yml`.
