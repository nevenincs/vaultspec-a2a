...
modified: '2026-09-21'
body_schema: body-v2
body_hash: 'sha256:0e2d4c204532c5b1f257d204098a6b9a0cc0cf09a191c62e4b756426ea7a8f54'
---
...
- [ ] `P01.S16` - Eliminate the HIGH CI-blocking canonical-api-cluster-isolation failure: two canonical runs recorded 40 failure marks across five API files while 136/136 serial and the xdist subset passed, with no traceback because a later timeout aborted; first eliminate or avoid the shared deadlock, rerun a captured -x canonical prefix to obtain the first traceback, then fix the real shared state, environment, or runner isolation without serializing the whole suite or weakening assertions, preserving durable CI log/session evidence and cleanup; `src/vaultspec_a2a/api/tests/ five affected files named by the first captured -x traceback, shared state/environment/runner helpers only when that trace proves ownership, canonical CI runner/session logs, temporary resources and cleanup evidence`.
