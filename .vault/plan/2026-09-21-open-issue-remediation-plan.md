...
modified: '2026-09-22'
body_schema: body-v2
body_hash: 'sha256:70644d74d22a28f9272aeb15fbb89aa4a6346277150b96ff416f0778f1018b5a'
---
...
- [x] `P01.S01` - Certify the lifecycle-readiness and trace slice of issue #18, and retain the broader containerization/BOLID scope as an open successor queue item; `src/vaultspec_a2a/lifecycle/manager.py, src/vaultspec_a2a/lifecycle/tests/, src/vaultspec_a2a/service_tests/test_cancel_health_trace.py, GitHub issue #18`.
...
- [x] `P01.S04` - Deliver the bounded A2A release-preparation guarantees for issue #26 while retaining its broader release-please and first-release scope as an open successor queue item; `.github/workflows/release.yml, pyproject.toml, CHANGELOG.md, scripts/, GitHub issue #26`.
...
- [x] `P01.S15` - Investigate and fix the HIGH canonical-dispatch-id-concurrency-deadlock, keeping test-infrastructure versus correctness classification open until proof: reproduce the deterministic full-prefix stall at worker/tests/test_dispatch_ids.py:179 with two ThreadPoolExecutor TestClient requests blocked through the AnyIO portal while the test holds the executor ingest lock; bound the root cause and apply a non-timeout fix that preserves identical-capacity exactly-once behavior, with durable CI log/session evidence and cleanup; `src/vaultspec_a2a/worker/tests/test_dispatch_ids.py:179, production or helper paths only after the root cause proves they are involved, canonical CI runner/session logs, temporary resources and cleanup evidence`.
- [x] `P01.S16` - Eliminate the HIGH CI-blocking canonical API cluster-isolation failure by fixing the test bootstrap ordering: testing/__init__.py:21-25 eagerly imports environment/settings before root conftest.py sets VAULTSPEC_ENVIRONMENT=development, so the canonical serial runner constructs Settings with the environment undeclared, /internal/events returns 500, and 40 API tests fail; xdist inherits the later declaration. Capture a fresh -x canonical prefix, correct testing runner/bootstrap isolation without serializing the suite or weakening assertions, preserve fail-closed production policy and avoid global environment leakage, and retain durable log/session cleanup evidence; `src/vaultspec_a2a/testing/__init__.py, src/vaultspec_a2a/testing/runner_child.py, src/vaultspec_a2a/testing/tests/test_runner.py, src/vaultspec_a2a/api/tests/test_internal.py, src/vaultspec_a2a/api/tests/test_internal_auth.py, src/vaultspec_a2a/api/tests/test_active_run_discovery_live.py, src/vaultspec_a2a/api/tests/test_cancel_settled_run_status.py, src/vaultspec_a2a/api/tests/test_gateway_drain.py`.
...
- [x] `P02.S09` - Perform the integrated architecture and code review, classify every finding, append the rolling audit queue, and reconcile all issue dispositions; `.vault/audit/, .vault/plan/2026-09-21-open-issue-remediation-plan.md, GitHub open-issue backlog`.
