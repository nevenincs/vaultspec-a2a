...
modified: '2026-09-23'
body_schema: body-v2
body_hash: 'sha256:8f0405acef308b4336d132d5644457db09c10216032d9dccc86e8a79d2ecb669'
---
...
- [ ] `P01.S01` - Resolve the project root by override, marker search, then working directory, split it from the install root, and read .env from it; `src/vaultspec_a2a/control/`.
- [ ] `P01.S02` - Rename every a2a settings field to the VAULTSPEC_A2A_ prefix with foreign-tool fallbacks and one name registry for child environments; `src/vaultspec_a2a/`.
- [ ] `P01.S03` - Enroll every raw environment read as a settings field and route consumers through settings; `src/vaultspec_a2a/`.
...
- [ ] `P02.S05` - Derive one state layout for default and desktop homes, default the home to .vault/data/agents, and resolve relative storage paths against the project root; `src/vaultspec_a2a/control/`.
- [ ] `P02.S06` - Route logs, discovery, singleton, process registry, worker-log sweep, provider temp homes and engine discovery through the layout; `src/vaultspec_a2a/`.
...
- [ ] `P03.S08` - Give every pytest session a unique worktree root holding basetemp, state home and process registry; `src/vaultspec_a2a/testing/`.
