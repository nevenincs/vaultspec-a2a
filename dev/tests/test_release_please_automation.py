...
def test_release_please_config_tracks_the_python_package_version() -> None:
    """The release PR advances its package, manifest, and editable lock entry."""
    config = json.loads(
    ...
    assert package["extra-files"] == [
        {
            "type": "toml",
            "path": "uv.lock",
            "jsonpath": "$.package[?(@.name == 'vaultspec-a2a')].version",
        }
    ]
...
def test_release_please_runs_only_for_main_and_uses_the_pr_gate() -> None:
    """The proposal is serialized and leaves the required check to PR events."""
...
def test_release_please_cannot_dispatch_or_weaken_the_artifact_publisher() -> None:
    ...
    assert "gh workflow run release.yml" not in release_please
    assert "gh workflow run" not in release_please
