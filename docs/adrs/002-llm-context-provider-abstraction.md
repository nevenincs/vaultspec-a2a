...
* **ACP-LangChain Wrapper Architecture (`AcpChatModel`):** The
  ...
  * **For Gemini:** `shutil.which("gemini")` resolves `gemini.exe`
    (Gemini deploys as a plain `.exe`, not a `.cmd` shim —
    `shutil.which` is safe). The ACP adapter is invoked as
    `gemini --experimental-acp` under the resolved binary path.
...
* **`shutil.which` for Claude:** Rejected. Claude does not deploy as
  ...
  `shutil.which("claude")` resolves a `.CMD` shim, which is banned. The
  raw `dist/index.js` path must be resolved from the npm package
  location.
