...
3. **Persistence Layer:** **SQLite** accessed asynchronously via `aiosqlite`.
   ...
   sourcing writes.
4. **Frontend Delivery (Static SPA):** The SvelteKit frontend will be compiled
   into a static Single Page Application (HTML/CSS/JS) via `vite build`.
5. **Deployment Model (`pip install`):** The compiled SvelteKit static assets
...
* **The "Jupyter" Deployment Pattern:** By compiling the SvelteKit app to
...
* **Static Asset Caching:** When FastAPI serves the SvelteKit `build/`
...
* **Build Pipeline Complexity:** Developers must ensure the SvelteKit build
...
* **SvelteKit Deployment:**
  [Svelte.dev Deployment](https://svelte.dev/docs/kit/adapter-static)
