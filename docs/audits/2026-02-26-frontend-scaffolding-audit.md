...
1. **H-4**: Implement sidebar with shadcn Sidebar, thread list, team status
2. **H-5**: Create unified chronological stream interleaving messages + tool
   calls + artifacts
3. **H-6**: Inline thread creation from input bar
4. **H-2 + H-3**: Use PermissionModal in thread page, make non-dismissible
5. **M-10**: Wrap TeamStatusPanel tooltips in Tooltip.Provider
...
1. Status bar (connection indicator, agent count, heartbeat)
2. Theme toggle with mode-watcher (Dark/Light/System)
3. Keyboard shortcuts (Ctrl+K, Ctrl+N, Ctrl+Enter, Escape)
4. Smart auto-scroll
5. Inspector panel with tabbed content
6. Input bar with provider/model selector + Stop button
7. Wire ErrorEvent → sonner toast
8. Fix type issues (M-1, M-2, M-9)
9. Add ESLint config for`src/ui/`
10. Fix stores barrel re-exports
11. Add unit tests for stores and WebSocket client
