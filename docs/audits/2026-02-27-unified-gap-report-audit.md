...
**Implementation
Reality**:`compile_team_graph()`in`lib/core/graph.py`uses`MemorySaver()`as the
...
**Implementation Reality**: `lib/api/websocket.py:268-277`logs the WS permission
...
**Implementation
Reality**:`lib/api/app.py`and`lib/api/endpoints.py`use`asyncio.create_task()`for
...
**ADR Intent**: The probe (`lib/providers/probes/_protocol.py`) and the
...
and architecturally sound. Added as L6 (LOW severity letter-of-law deviation).
-`lib/core/config.py`does not have an explicit`__all__` declaration. Its exports
(`Settings`, `settings`) are re-exported by `lib/core/__init__.py`. Added as L7
...
- L12: CORS uses wildcard`*`instead of plan-specified`localhost:5173`. Low risk
  for dev tool.
- ~65 individual PASS checks verified across lib/api/, lib/database/,
  lib/telemetry/, lib/workspace/.
