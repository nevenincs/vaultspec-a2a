...
| ADR-020 | Blackboard Content Mounting | Approved, on disk |
| ADR-021 | Persistent Task Queue Schema | Approved, on disk |
| ADR-023 | Phase Artifact Gates | Approved, on disk |
| ADR-024 | Plan Approval Interrupt | Approved, on disk |
| ADR-025 | Mandatory Review Gate | Approved, on disk |
...
|----|--------|-------------|-----------|
| TRADEOFF-01 | ADR-026 | `pipeline_phase` one-invocation lag: `infer_phase_from_vault_index` computes the phase before `build_anchoring_context`, but the value only enters `TeamState` via the supervisor's return dict. Anchoring reads the *previous* invocation's `pipeline_phase`. | Same class as transition-moment lag (ADR-026 §2.5). Fixable by passing inferred phase to `build_anchoring_context` as parameter override (changes ADR-022 signature). Deferred — one-invocation lag is acceptable for v1. |
