...
- **4 app-owned tables**: `threads`, `artifacts`, `permission_logs`, `cost_tracking` (all in `lib/database/models.py` via `Base.metadata`)
...
- **Existing data migration**: `backfill_teamstate_sdd_fields()` in `lib/database/migrations/__init__.py` — out of scope, keep as-is
...
2. **Create `alembic.ini`** at repo root with `script_location = lib/database/migrations` and `sqlalchemy.url = sqlite+aiosqlite:///vaultspec.db`
3. **Create `lib/database/migrations/env.py`** — canonical async pattern from research doc section 3, with:
...
4. **Create `lib/database/migrations/script.py.mako`** — standard Alembic template
5. **Create `lib/database/migrations/versions/` directory** (empty `__init__.py`)
...
11. **Add `run_migrations()` helper** to `session.py` (or a new `lib/database/migrate.py`):
    ```python
    ...
    ```
12. **Wire startup**: call `run_migrations()` in `lib/api/app.py` lifespan before first request (gated by `settings.auto_migrate` flag, default `True` for dev)
...
15. **Add migration test**: new test in `lib/database/tests/test_migrations.py` that:
...
17. **Update `__all__`** in `lib/database/session.py` if `run_migrations` is added there
18. **Add `auto_migrate` setting** to `lib/core/config.py` Settings model
...
| `lib/database/migrations/env.py` | New — async Alembic env |
| `lib/database/migrations/script.py.mako` | New — template |
| `lib/database/migrations/versions/` | New directory |
| `lib/database/migrations/versions/0001_initial_schema.py` | New — autogenerated |
| `lib/database/session.py` | Refactor `init_db()`, add `run_migrations()` |
| `lib/api/app.py` | Wire `run_migrations()` in lifespan |
| `lib/core/config.py` | Add `auto_migrate: bool = True` |
| `lib/database/tests/test_migrations.py` | New — migration tests |
...
1. **`alembic.ini` location**: Repo root (conventional) vs `lib/database/` (co-located). Recommendation: repo root.
