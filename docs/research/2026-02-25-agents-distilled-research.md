...
- **`ANTHROPIC_API_KEY`**: Standard pay-as-you-go API key. Must be **unset**
  when
...
- **`OPENAI_API_KEY`**: Standard pay-as-you-go API key for headless
  environments.

**Canonical path**: Browser sign-in once to generate cached session token;
inject
...
heterogeneity is no longer a concern.
...
resolves reliance on interactive auth flows.
...
**Status**: Resolved by ADR-001 & ADR-008. Subprocesses and provider CLIs have
been abandoned entirely in favor of native Python execution via LangGraph,
neutralizing Windows CLI limitations.
...
**Status**: Resolved by ADR-002. Persistent setup tokens are mandated to prevent
session expirations, offloading refresh logic entirely.
...
**Status**: Mitigated by ADR-002. LangGraph state checkpointing drastically
reduces context window bloat, artificially expanding the effective throughput
under existing rate limits.
...
**Status**: Resolved by ADR-006. ACP support has been abandoned completely from
the architecture.
...
**Status**: Resolved by ADR-002 & ADR-006. A2A is abandoned. GLM-5's OpenAI
compatibility seamlessly handles tool-calling translation natively within
