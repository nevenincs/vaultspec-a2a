...
## Issue 1: Event Queue Backpressure — Blocking Put vs Oldest-Message-Drop (ROB-002 / A2A-003)
...
### Key Pattern
**A2A also uses blocking `await queue.put(event)`.** The SDK's `EventQueue` uses a
...
when the queue is full, effectively stalling the producer until space is available.
...
SDK has no lifespan handler at all in its FastAPI apps (`A2AFastAPIApplication.build()`
...
2. **Application-level state counter**: For domain-specific loop control (like
...
2. Use environment-configurable origins:
   ...
   ```
3. For development, allow localhost origins. For production, restrict to the
