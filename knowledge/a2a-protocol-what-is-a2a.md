...
The diagram shows a linear flow starting from a **User**, who provides a **Prompt**, which is then received by an **AI Assistant**.
...
The diagram illustrates the **AI Assistant** at the center, acting as a hub that connects to a subgraph of **Specialized Agents**. The assistant initiates connections to:
...
The diagram illustrates the interactions between a **Client**, an **A2A Server**, and an **Auth Server** across four distinct phases:

1. **Agent Discovery:**
    * The Client sends a `GET` request for the agent card (e.g., to `/.well-known/agent-card`) to the A2A Server.
    * The A2A Server returns the **Agent Card** to the Client.

2. **Authentication:**
    * The Client parses the Agent Card for `securitySchemes`.
    * If the `securityScheme` is "openIdConnect", the Client requests a token from the **Auth Server** using the provided `authorizationUrl` and `tokenUrl`.
    * The Auth Server returns a **JWT** (JSON Web Token) to the Client.

3. **sendMessage API:**
    * The Client parses the Agent Card for the `url` parameter to identify the API endpoint.
    * The Client sends a `POST /sendMessage` request (including the JWT) to the A2A Server.
    * The A2A Server processes the message, creates a task, and returns a **Task Response** to the Client.

4. **sendMessageStream API:**
    * The Client sends a `POST /sendMessageStream` request (including the JWT) to the A2A Server.
