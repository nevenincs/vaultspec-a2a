"""Worker process -- standalone FastAPI application (ADR-019).
...
Run standalone::
    ...
    python -m uvicorn lib.worker.app:create_worker_app --factory \\
...
async def _lifespan(app: FastAPI) -> AsyncGenerator[None]:
    ...
    async with AsyncSqliteSaver.from_conn_string(str(db_path)) as checkpointer:
        ...
        bridge = WorkerBridge(settings.api_base_url, worker_id)
...
def main() -> None:
    ...
    uvicorn.run(
        "lib.worker.app:create_worker_app",
        factory=True,
        host="127.0.0.1",
