...
export function agentStateColor(state: AgentLifecycleState): string {
  switch (state) {
    ...
    case 'idle': return 'text-status-success';
    case 'working': return 'text-status-info';
    case 'input_required': return 'text-status-warning';
    case 'auth_required': return 'text-status-warning';
...
export function agentStateDot(state: AgentLifecycleState) {
  switch (state) {
    ...
    case 'submitted':
      return (
        <Loader2 className="w-3.5 h-3.5 text-status-info animate-spin" />
      );
    case 'input_required':
    case 'auth_required':
      return (
    ...
    case 'failed':
    case 'cancelled':
      return <span className="w-2.5 h-2.5 rounded-full bg-status-error" />;
    case 'completed':
    case 'idle':
      return null;
  }
...
export function agentStateLabel(state: AgentLifecycleState): string {
  switch (state) {
    ...
    case 'input_required': return 'input needed';
...
export function toolStatusColor(status: ToolCallStatus): string {
  switch (status) {
    ...
    case 'running': return 'text-status-info';
    case 'completed': return 'text-status-success';
    case 'failed': return 'text-status-error';
  }
}
