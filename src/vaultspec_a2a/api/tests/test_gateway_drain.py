...
async def test_run_start_admits_while_open_then_refuses_once_draining(
    ...
    async with (
        ...
        first = await client.post("/v1/runs", json=await _run_body(client))
        ...
        refused = await client.post("/v1/runs", json=await _run_body(client))
...
async def test_unexpected_run_start_failure_releases_admission_and_drain_quiesces(
    ...
    try:
        ...
        async with (
            ...
            # The admission was released on the failure path: no phantom active
            # run. Reached through the production accessor rather than
            # `app.state.drain_gate`, which only exists once something has been
            # admitted - the refusal now happens before admission, so reading
            # the attribute asserted a side effect instead of the invariant.
    ...
    async with httpx.AsyncClient(
        ...
        async with (
            ...
            resp = await client.post(
                "/v1/runs", json=await _run_body(client, run_id=run_id)
            )
