...
def _valid_kwargs() -> dict:
    return {
        ...
        "counters": ProgressCounters(tool_calls=2, artifacts=1, messages=4),
...
def test_a_valid_positive_frame_constructs_and_round_trips() -> None:
    frame = PositiveProgressEvent(**_valid_kwargs())
    ...
    assert PositiveProgressEvent(**dumped) == frame
...
def test_a_forbidden_field_cannot_be_smuggled_into_the_frame(forbidden: str) -> None:
    ...
    with pytest.raises(ValidationError):
        PositiveProgressEvent(**_valid_kwargs(), **{forbidden: "leaked content"})


def test_the_token_delta_is_bounded() -> None:
    with pytest.raises(ValidationError):
        PositiveProgressEvent(**{**_valid_kwargs(), "token_delta": 10**8})
    with pytest.raises(ValidationError):
        PositiveProgressEvent(**{**_valid_kwargs(), "token_delta": -1})
