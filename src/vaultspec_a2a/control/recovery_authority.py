...
async def reconcile_run_checkpoint(
    ...
    *,
    ...
    evidence = await read_checkpoint_evidence(
        checkpointer, receipt, timeout_seconds=checkpoint_timeout_seconds
    )
    if evidence.kind is not CheckpointEvidenceKind.COMPLETED:
        if trigger is RecoveryTrigger.STARTUP and status not in {
            ...
            election = await elect_thread_status(
                ...
                successor=successor_thread_write_authority(
                    ...
                    action_receipt_id=receipt.dispatch_id,
                ),
            )
            ...
            await db.commit()
            fresh = await db.get(ThreadModel, thread_id, populate_existing=True)
