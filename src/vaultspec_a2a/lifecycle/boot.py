...
def serve_env(role_cfg: RoleConfig, **options: Unpack[_ServeEnvArgs]) -> dict[str, str]:
    ...
    A recorded *engine_service_json* is injected under the
    ``engine_service_json`` setting's name so the worker's engine discovery no
    longer depends on the booting shell having exported it - the
    reseat-strands-worker gap.
