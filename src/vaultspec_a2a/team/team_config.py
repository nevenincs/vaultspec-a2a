...
def supported_capabilities(topology_type: TopologyType) -> list[str]:
    ...
    decision, and (agent-flow ADR D4) a plan document; coder topologies produce
    no vault-document capability under this mission surface. Diagnostic truth
    for the Rust backend, not product curation text.
    """
    if topology_type == TopologyType.RESEARCH_ADR:
